...
// Strips reply/forward prefix and takes up to maxWords whole words
...
export default function CSBoard({ data, loading }) {
  ...
  return (
    <>
      <BoardShell themeKey="engineer" data={data} loading={loading} onInboxEmailClick={setDraftEmail}>
        <div style={{ position: 'relative', width: '100%', height: '100%' }}>
          <ForceGraph nodes={nodes} edges={edges} theme={THEME} onNodeClick={setSelected} />
          <NodeDetail node={selected} accent="#2563EB" onClose={() => setSelected(null)} onDraftReply={setDraftEmail} />
        </div>
      </BoardShell>
      {draftEmail && (
        <DraftCompose email={draftEmail} accent="#2563EB" onClose={() => setDraftEmail(null)} />
      )}
    </>
  );
}
  ...
  return (
    <div style={{ width:'100vw', height:'100vh', background:'#EFF4FF', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:"'DM Sans',system-ui,sans-serif" }}>
      <div style={{ textAlign:'center' }}>
        <div style={{ fontSize:9, letterSpacing:'2.5px', textTransform:'uppercase', color:'rgba(37,99,235,0.45)', marginBottom:10 }}>juno</div>
